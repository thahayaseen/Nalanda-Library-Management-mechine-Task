...
export class nodemaileService implements Inodemailservices{
 ...
 private getOtpDatas(otp: string, email: string, name: string) {
 ...
 <body>
  ...
  async sendOtp(otp: string, email: string, name: string):Promise<void> {
    const { html, subject, text } = this.getOtpDatas(otp, email, name);
    await this.nodeMailerProvider.sendMail(email, subject, text, html);
    return
  }
}
